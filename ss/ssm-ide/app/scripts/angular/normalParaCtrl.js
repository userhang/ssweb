 ...
 */
/*********************************************
 * 描述：普通段落（normalPara）的设置方法的参数设置
 *********************************************/

var app = angular.module('app');
app.controller('normalParaCtrl', ['$scope', '$location',function normalParaCtrl($scope, $location) {
    ...
    //更改当前段落内容
    $scope.changeNormalParaContent=function changeNormalParaContent(){
        var data = {
            'seq':$scope.seq,
            'directive':'changeNormalParaContent',
            'attr':{'normalParaContent':$scope.normalParaContentInput}
        };
        sendMessage_funcdirective(data);
    };

    //修改文字字体
    $scope.changeNormalParaContentFontFamily=function changeNormalParaContentFontFamily(){
        var data = {
            'seq':$scope.seq,
            'directive':'changeNormalParaContentFontFamily',
            'attr':{'normalParaContentFontFamily':$scope.normalParaContentFontFamilySelected}
        };
        sendMessage_funcdirective(data);
    };

    //修改当前段落高度
    $scope.changeNormalParaHeight=function changeNormalParaHeight(){
        var data = {
            'seq':$scope.seq,
            'directive':'changeNormalParaHeight',
            'attr':{'normalParaHeight':$scope.normalParaHeightInput}
        };
        sendMessage_funcdirective(data);
    };
    ...
    $scope.isShow = function isShow(flag){
            ...
            var matchingText = $scope.seq + 'normalParaEventSelect';
...
//适配函数
    $scope.addinitjs=function addinitjs(){
     ...
     //新增js事件绑定
    $scope.addnormalParaevent=function addnormalParaevent(){

            tmpeventdata={
                'directive':'addeventbind',
                'eventid':Math.uuid(),
                'seq':$scope.seq,
                'childattr':{'normalParaContent':$scope.normalParaContentInput},
                'funcType':$scope.normalParaeventselect,
                'funcContent':"",
                'flag':""
             
            };
          
        tmpeventdata.flag = $scope.seq + 'normalParaEventSelect';
        //在这里全局变量就已经被改变了
...
//            }
    ...
    //编辑js事件绑定
    $scope.editnormalParaevent=function editnormalParaevent(elemEvent){
    ...
    //删除js事件绑定
    $scope.delnormalParaevent=function delnormalParaevent(elemEvent){
        eventData={
    ...
    //默认事件选择为第一个
    $scope.normalParaeventselect=$scope.normalParaevent[0];
    ...
    $scope.$watch('attr', function() {
         //当前id
         $scope.normalParaidInput=$scope.attr.normalParaid;
        //当前段落说明文字
        $scope.normalParaContentInput=$scope.attr.normalParaContent;
        //文字字体
        if($scope.attr.normalParaContentFontFamily != null){
            for(var j = 0; j < $scope.componentFontFamily.length; j++){
                if($scope.attr.normalParaContentFontFamily.val == $scope.componentFontFamily[j].val){
                    $scope.normalParaContentFontFamilySelected = $scope.componentFontFamily[j];
                }
            }
        }
        //短落高度
        $scope.normalParaHeightInput=$scope.attr.normalParaHeight;
    },true);

    //界面分组的文字（第一组）
    $scope.nparaTitle = '一般段落设置';
      //界面分组文字（第二组）
    $scope.nparaeventbindTitle = '一般段落事件绑定';
    //界面分组文字（第三组）
    $scope.nparaeventeditTitle = '一般段落事件编辑';
}]);
